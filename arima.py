...
from darts.models import LightGBMModel, AutoARIMA, StatsForecastAutoARIMA
...
def holiday_list(df_stores, df_holidays_events):
    ...
    for store in range(len(df_stores)):
        ...
        holiday["work_day"] = np.where((df_holidays_events["type"] == "Work Day"), 1, 0)
...
def LGBM_model_training(family_list, ts_fam_transform_dict, transactions_transform, future_covar_dict):
    ...
    for family in tqdm(family_list):
        ...
        LGBM_training_model_per_family=[]
        for i in range(len(sales_family)):
            ...
            training_slice = sales_family[i].slice_intersect(TCN_covar[i])
            LGBM_training_model_per_family_per_store = StatsForecastAutoARIMA(start_p=8, max_p=9)

            print("here")
            LGBM_training_model_per_family_per_store.fit(series=training_slice, 
                            future_covariates=TCN_covar[i]
                            )
            LGBM_training_model_per_family.append(LGBM_training_model_per_family_per_store)
        LGBM_training_models[family] = LGBM_training_model_per_family
    #returning a dict of lists that contain all the models of that store
    return LGBM_training_models
...
def LGBM_model_predicting(family_list, ts_fam_transform_dict, transactions_transform, future_covar_dict, LGBM_training_models):
    ...
    for family in tqdm(family_list):
        ...
        TCN_covar = future_covar_dict[family]
        LGBM_training_models_per_store=LGBM_training_models[family]
        LGBM_predicting_model_per_family=[]
        for i in range(len(sales_family)):
            training_slice = sales_family[i].slice_intersect(TCN_covar[i])
            LGBM_predicting_model_per_family_per_store = LGBM_training_models_per_store[i].predict(n=16,
                                                        future_covariates=LGBM_covar[i]
                                                        )
            LGBM_predicting_model_per_family.append(LGBM_predicting_model_per_family_per_store)                                        
...
def main():
    ...
    LGBM_training_models = {}
    LGBM_training_models = LGBM_model_training(family_list, ts_fam_transform_dict, 
